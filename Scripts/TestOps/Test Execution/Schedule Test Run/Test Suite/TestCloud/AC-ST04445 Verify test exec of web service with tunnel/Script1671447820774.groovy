...
if (GlobalVariable.isNewUIScheduleEnabled) {
	...
	Page.nav(NewUIScheduleTestRunPopUp)
		.inputTestRunName("ws-${browserName}-${browserVersion}-${os}-private-${unique}")
		.clickScriptRepo()
...
Page.nav(TestRunListPage).filterByName("ws-${browserName}-${browserVersion}-${os}-private-${unique}")

'Verify all test run information of the first row on the list is correct'
Page.nav(TestRunListPage)
	.verifyTestRunName(1, "ws-${browserName}-${browserVersion}-${os}-private-${unique}")
