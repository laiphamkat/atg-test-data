...
Page.nav(NewUIScheduleTestRunPopUp)
	.inputTestRunName("Mobile-browser-public-${platform}-${unique}")
	.clickScriptRepo()
...
Page.nav(TestRunListPage).filterByName("Mobile-browser-public-${platform}-${unique}")

'Verify all test run information of the first row on the list is correct'
Page.nav(TestRunListPage).verifyTestRunName(1, "Mobile-browser-public-${platform}-${unique}")
...
Page.nav(TestRunsPage)
	.verifyTestRunName(1, "Mobile-browser-public-${platform}-${unique}")
