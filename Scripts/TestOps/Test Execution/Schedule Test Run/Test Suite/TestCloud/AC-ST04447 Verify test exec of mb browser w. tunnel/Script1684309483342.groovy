...
Page.nav(NewUIScheduleTestRunPopUp)
	.inputTestRunName("Mobile-browser-private-${platform}-${unique}")
	.clickScriptRepo()
...
Page.nav(TestRunListPage).filterByName("Mobile-browser-private-${platform}-${unique}")

'Verify all test run information of the first row on the list is correct'
Page.nav(TestRunListPage).verifyTestRunName(1, "Mobile-browser-private-${platform}-${unique}")
...
Page.nav(TestRunsPage)
	.verifyTestRunName(1, "Mobile-browser-private-${platform}-${unique}")
