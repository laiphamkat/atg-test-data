...
'Verify custom field are assigned to test schedule succesffuly'
for (int i=0; i < lstCustomFields.size(); i++) {
	Page.nav(NewUIScheduleTestRunPopUp).verifyCustomFieldDisplay(lstCustomFields[i].displayName, lstCustomFields[i].customFieldOptions.get(0).value)
}