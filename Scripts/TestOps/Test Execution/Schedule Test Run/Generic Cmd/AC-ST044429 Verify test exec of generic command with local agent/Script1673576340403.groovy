...
if (GlobalVariable.isNewUIScheduleEnabled) {
	...
	Page.nav(NewUIScheduleTestRunPopUp)
		.inputTestRunName("generic-command-local-agent-${unique}")
		.clickScriptRepo()
...
Page.nav(TestRunListPage).filterByName("generic-command-local-agent-${unique}")

'Verify all test run information of the first row on the list is correct'
Page.nav(TestRunListPage)
	.verifyTestRunName(1, "generic-command-local-agent-${unique}")
	.verifyTestRunRelease(1, '')
	.verifyTestRunScriptRepo(1, 'cloudian-automation-1')
	.verifyTestRunTestEnv(1, 'cloudian-local-agent-1')
...
Page.nav(TestRunDetailPage)
	...
	.verifySessionTestEnv(1, 'cloudian-local-agent-1')
...
def tearDownIfPassed() {	
	SeedingData.saveTestExecToGSheet('TEST-EXEC-GENERIC-CMD-LOCAL-AGENT-1')
}	
